...
export const PaymentsReadOnlyPage: React.FC = () => {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t("common.errors.loadError"));
      setRows([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-full overflow-x-hidden bg-[#f8fafc] pb-[110px] text-[#334155] max-md:[&_button]:min-h-[44px] md:pb-0">
      <AppContainer className="max-w-6xl space-y-6">
        <PageHeader
          ...
          subtitle={t("billing.payments.subtitle")}
          actions={
        ...
        >
          ...
          {t("billing.payments.backToInvoices")}
        </Link>
        ...
        <FiltersBar className="md:grid-cols-3">
          ...
          >
            <option value="">{t("common.filters.allMethods")}</option>
        ...
        <DataTable
          title={t("billing.payments.historyTitle")}
          subtitle={t("billing.payments.historySubtitle")}
          loading={loading}
          empty={!loading && filteredRows.length === 0}
          emptyTitle={rows.length === 0 ? t("billing.payments.emptyNoPayments") : t("billing.payments.emptyFiltered")}
          emptySubtitle={
            rows.length === 0 ? t("billing.payments.emptyHintNoPayments") : t("billing.payments.emptyHintFiltered")
          }
        >
            ...
            <table className="w-full min-w-[980px] text-sm">
              <thead className="bg-[#f8fafc]">
                <tr className="border-b border-[#e5e7eb]">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#64748b]">{t("common.table.patient")}</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#64748b]">{t("common.table.invoice")}</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[#64748b]">{t("common.table.method")}</th>
